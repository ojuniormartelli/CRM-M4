...
const Clients: React.FC<ClientsProps> = ({ clients, setClients, currentUser, workspaceId }) => {
  ...
  const filteredClients = clients.filter(c => {
    ...
    const matchesStatus = showExClients ? c.status === 'churned' : c.status !== 'churned';
      ...
      <div className="flex justify-between items-center">
        <div>
          <h2 className="text-3xl font-black text-slate-900 dark:text-white tracking-tight">
            {showExClients ? 'Ex-Clientes' : 'Base de Clientes'}
          </h2>
          <p className="text-slate-500 dark:text-slate-400 font-medium">
            {showExClients ? 'Histórico de contas encerradas.' : 'Gestão de contas e relacionamento operacional.'}
          </p>
        </div>
        <div className="flex items-center gap-4">
          <button
            onClick={() => setShowExClients(!showExClients)}
          ...
          >
            {showExClients ? 'Ver Ativos' : 'Ver Ex-Clientes'}
          </button>
            ...
            <div className="flex items-center gap-4 mb-6">
              ...
              <div>
                ...
                <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">
                  {client.status === 'churned' ? 'Ex-Cliente' : 'Cliente Ativo'}
                </p>
              </div>
            </div>
              ...
              <div className="flex items-center gap-3">
                {client.status !== 'churned' && (
                  <button 
                    ...
                    className="text-amber-600 dark:text-amber-400 text-[10px] font-black uppercase tracking-widest hover:underline disabled:opacity-50"
                  >
                    Arquivar
                  </button>
                )}
                <button 
                  ...
                  disabled={isProcessing === client.id}
                  className="text-rose-600 dark:text-rose-400 text-[10px] font-black uppercase tracking-widest hover:underline disabled:opacity-50"
                >
                  Excluir
                </button>
        ...
        {filteredClients.length === 0 && (
          <div className="col-span-full py-20 text-center space-y-4">
            ...
            <p className="text-slate-400 font-black uppercase text-xs tracking-widest">
              {showExClients ? 'Nenhum ex-cliente encontrado' : 'Nenhum cliente encontrado'}
            </p>
          </div>
        )}
      </div>

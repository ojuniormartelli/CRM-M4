    ...
    <div className="h-full overflow-y-auto pr-4 scrollbar-none space-y-10 animate-in fade-in duration-700">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-slate-500 font-medium">Gestão de contas e relacionamento.</p>
        </div>
        <div className="relative">
          ...
          <input 
            ...
            className="pl-12 pr-6 py-3 bg-white border border-slate-100 rounded-2xl font-bold text-sm focus:ring-2 focus:ring-blue-500 outline-none w-80 shadow-sm"
          />
        </div>
      </div>
        ...
        {filteredClients.map((client) => (
          <div key={client.id} className="bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm hover:shadow-xl transition-all group">
            <div className="flex items-center gap-4 mb-6">
              <div className="w-14 h-14 rounded-2xl bg-slate-50 flex items-center justify-center text-slate-400 font-black text-xl group-hover:bg-blue-600 group-hover:text-white transition-all">
            ...
            <div className="flex justify-between items-center pt-6 border-t border-slate-50">
              <span className={`text-[9px] font-black uppercase px-3 py-1 rounded-full ${
                client.status === 'active' ? 'bg-emerald-100 text-emerald-600' : 'bg-red-100 text-red-600'
              }`}>
                {client.status === 'active' ? 'Ativo' : 'Inativo'}
              </span>
              <button className="text-blue-600 text-[10px] font-black uppercase tracking-widest hover:underline">Ver Detalhes</button>
            </div>
          </div>
        ))}
